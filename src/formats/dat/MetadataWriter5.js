// Generation-5 (Tibia 8.55 – 9.86) DAT writer.
// AS3 reference: otlib.things.MetadataWriter5.
...
function writeLatin1String(w, s) {
    for (let i = 0; i < s.length; i++) {
        w.writeUint8(s.charCodeAt(i) & 0xFF);
    }
}

export function writeProperties(w, t) {
    if (t.category === ITEM) throw new Error("MetadataWriter5.writeProperties: item — use writeItemProperties");
...
export function writeItemProperties(w, t) {
    if (t.category !== ITEM) throw new Error("MetadataWriter5.writeItemProperties: non-item");
