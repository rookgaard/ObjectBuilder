// Generation-5 (Tibia 8.55 – 9.86) DAT reader. Adds CLOTH + MARKET_ITEM,
// plus translucent. From v ≥ 960, sprite ids become u32 ("extended").
// AS3 reference: otlib.things.MetadataReader5.

import F from "./MetadataFlags5.js";

export { readTexturePatterns } from "./MetadataReader.js";
export const GENERATION = 5;

// AS3 uses ISO-8859-1 — every byte ↦ same codepoint.
...
export function readProperties(reader, type) {
    ...
    throw new Error(`MetadataReader5: flag stream did not terminate on ${type.category} ${type.id}`);
}
